...
namespace SunnyFlamingo.Extensions
{
    ...
    {
        public static async Task<GoodsDbInformation> GetGoodsDbInformation(this ApplicationDbContext context, string commandText, params SqlParameter[] parameters)
        {
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                {
                    var questions = await MapToList<DBQuestionBase>(dataReader);
                    dataReader.NextResult();
                    var goodCells = await MapToList<GoodCellModel>(dataReader);
                    return new GoodsDbInformation(questions, goodCells);
                }
            }
        }
        private static async Task<List<T>> MapToList<T>(DbDataReader dataReader)
        {
            ...
            if (dataReader.HasRows)
            {
                ...
                {
                    columnNames.Add(dataReader.GetName(i).ToLower());
                }
                while (await dataReader.ReadAsync())
                {
                    ...
                    {
                        var val = dataReader.GetValue(columnNames.FirstOrDefault(cn => cn == prop.Name.ToLower()));
        ...
        public static async Task<List<IList>> MultiResultSetsFromSql(this ApplicationDbContext dbContext, ICollection<Type> resultSetMappingTypes, string sql, params object[] parameters)
        {
        ...
        public static async Task<(List<T1> FirstResultSet,
            List<T2> SecondResultSet)>

            MultiResultSetsFromSql<T1, T2>(
            ...
            params object[] parameters)
        {
            ...
            return ((List<T1>)resultSets[0], (List<T2>)resultSets[1]);
        }

        public static async Task<(IReadOnlyCollection<T1> FirstResultSet,
            IReadOnlyCollection<T2> SecondResultSet,
            IReadOnlyCollection<T3> ThirdResultSet)>

            MultiResultSetsFromSql<T1, T2, T3>(
            this ApplicationDbContext dbContext,
